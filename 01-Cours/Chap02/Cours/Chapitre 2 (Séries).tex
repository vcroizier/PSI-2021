...
%
%\noindent Pour tout $n \in \mathbb{N}$, on a :
...
%\bigskip
%
%\noindent Montrons maintenant que la série harmonique diverge. Pour tout $n \in \mathbb{N}^*$, on pose :
...
%
%\noindent En cas de convergence, en passant à la limite quand $n$ tend vers $+ \infty$ dans (\ref{reste}), on a :
...
%\noindent Les séries géométriques interviennent de manière important dans plusieurs chapitres. Le Théorème suivant (prouvé en plusieurs parties ci-dessus) est donc crucial : 
...
%
%\vspace{4cm}
%\noindent Raisonnons par l'absurde : si la série harmonique converge, la suite $(H_n)_{n \geq 1}$ est convergente donc la suite extraite de celle-ci $(H_{2n})_{n \geq 1}$ est aussi convergente et de même limite. Ainsi :
...
%
%\noindent Soit $f$ une fonction continue et décroissante sur $[n_0, + \infty[$ où $n_0 \in \mathbb{N}$. Fixons $k \in \mathbb{N}$ supérieur ou égal à $n_0$. Par décroissance de $f$ sur $[k,k+1]$, on a pour tout $x \in [k,k+1]$,
...
%\end{center}
%
%\noindent Soit $n \in \mathbb{N}$. En sommant l'inégalité de gauche de (\ref{SerieInt}) de $n_0$ à $n-1$ et l'inégalité de droite de $n_0$ à $n$, on obtient :
...
\noindent \textbf{Application : la série harmonique.} 
...
% \end{equation}
% 
%\noindent Or $\ln(n+1) \underset{+ \infty}{\sim} \ln(n)$ (on a $\ln(n+1) = \ln(n) + \ln \left(1+ \frac{1}{n}\right)$ et $\ln \left(1+ \frac{1}{n}\right) = o(1)$) et $1 + \frac{1}{\ln(n)}$ tend vers $1$ quand $n$ tend vers $\infty$. Par Théorème d'encadrement, on a alors par (\ref{Serihar}) :
...
%\medskip
%
%\noindent \textbf{Application 2 : trouver un équivalent de $\Sum{k = 2}{n} \dfrac{\ln(k)}{k}$ quand $n$ tend vers $+\infty$.}
%
%\vspace{10cm}
%\noindent La fonction $x \mapsto \frac{\ln(x)}{x}$ est décroissante (simple étude de fonction) et continue sur $[2, + \infty[$. Par comparaison série-intégrale, on a pour tout $n \geq 2$,
...
%
%\noindent Ainsi les deux séries sont de même nature.
%\end{enumerate}
\end{preuve}
...
\vspace{3cm}
%\noindent Cette série est à termes positifs. On a :
...
\noindent On suppose que $\left( \dfrac{u_{n+1}}{u_n} \right)_{n \geq 0}$ possède une limite $\ell$ (éventuellement $\ell = + \infty$). Alors :
...
\vspace{3cm}
%\noindent Soit $a \in \mathbb{R}_{+}^*$. La série étudiée est à termes strictement positifs. On a pour tout $n \in \mathbb{N}$,
...
%
%\noindent $\rhd$ Supposons que la suite $(u_n)_{n \geq 0}$ soit réelle et que $\Sum{n \geq 0}{} u_n$ converge absolument. Posons pour tout entier $n \geq 0$,
...
%\medskip
%
%\noindent $\rhd$ Supposons que $\Sum{n \geq 0}{} u_n$ est absolument convergente. Rappelons que pour tout $z \in \mathbb{C}$,
...
\vspace{6cm}
%
%\noindent $\rhd$ Supposons que $u_n =O(v_n)$. Par définition, il existe une suite réelle $(\theta_n)_{n \geq 0}$ bornée et un rang $N \in \mathbb{N}$ tel que pour tout entier $n \geq N$,
%$$ u_n = \theta_n v_n $$
...
%\medskip
%
%\noindent $\rhd$ Si $u_n = o(v_n)$ alors $u_n = O(v_n)$.
...
%
%\noindent La série n'est pas à termes positifs. Par développement asymptotique, on a :
...
\noindent $\rhd$ Pour montrer que la série converge, nous allons montrer que la suite des sommes partielles $(S_n)_{n \geq 0}$ converge en prouvant que ses deux suites extraites $(S_{2n})_{n \geq 0}$ et $(S_{2n+1})_{n \geq 0}$ convergent vers la même limite. 
%
%Pour tout $n \in \mathbb{N}$, on a :
...
\noindent $\rhd$ Démontrons la deuxième partie du théorème : l'estimation et le signe des restes. 
...
\vspace{6.5cm}
%
%\noindent $\rhd$ Si $z=0$, le terme général de la série est nul excepté pour $k=0$ où le terme vaut $1$. Dans ce cas la série est bien absolument convergente.
%
%\medskip
%
...
%
%\noindent Donnons les premières décimales de $\pi$ :
%
...
%\medskip
%
%\noindent Le problème de ce type d'écriture est qu'il n'y a pas l'unicité : 
%$$ 1.00000 \cdots = 0. 999999 \cdots$$
%
%\medskip
%
%\noindent On montre le résultat suivant :
%
...
%
%\noindent En posant $y=0.111111 \cdots$, on a $10y=1+y$ ou encore $y=\dfrac{1}{9}$ et ainsi $x= 0.2017 + \dfrac{y}{10^5}$ et finalement :
