...
\maketitle{Chapitre 7}{Fonctions vectorielles et arcs paramétrés}


\noindent Dans tout le chapitre, $I$ est un intervalle de $\mathbb{R}$ contenant au moins deux points et $n$ et $m$ sont deux entiers naturels non nul. Sauf indication contraire, $f$ désignera une application définie sur $I$ et à valeurs dans $\mathbb{R}^n$.

\medskip

\noindent Le but du chapitre est :

\begin{itemize}
\item de généraliser aux fonctions à valeurs dans $\mathbb{R}^n$ la notion de continuité et de dérivabilité d'une fonction numérique.
\item de formaliser certaines notions géométriques utilisées dans d'autres matières scientifiques.
\end{itemize}

%\section{Norme sur $\mathbb{R}^n$}
%\subsection{définition de la norme euclidienne}
%\noindent Pour tout $x=(x_1, \ldots,x_n) \in \mathbb{R}^n$, on pose :
%$$ \Vert x \Vert = \sqrt{x_1^2+ x_2^2+ \cdots + x_n^2}$$
%\begin{prop} L'application $\Vert \cdot \Vert : \mathbb{R}^n \rightarrow \mathbb{R}_+$ est appelée \textit{norme euclidienne} sur $\mathbb{R}^n$. Elle vérifie les propriétés suivantes :
%\begin{itemize}
%\item Pour tout $x \in \mathbb{R}^n$, $\Vert x \Vert =0 \Rightarrow x=0_{\mathbb{R}^n}$ (\textit{séparation}).
%\item Pour tout $x \in \mathbb{R}^n$ et tout $\lambda \in \mathbb{R}$, $\Vert \lambda x \Vert = \vert \lambda \vert \Vert x \Vert$ (\textit{homogénéité}).
%\item Pour tout $(x,y) \in (\mathbb{R}^n)^2$, $\Vert x+y \Vert \leq \Vert x \Vert + \Vert y \Vert$ (\textit{inégalité triangulaire})
%\end{itemize}
%\end{prop}
%
%\begin{preuve} Voir le chapitre sur les espaces vectoriels normés.
%\end{preuve}

%\subsection{Limite d'une fonction}
%\begin{defin} Soit $a \in I$ ou une extrémité de $I$. On dit que $f$ admet pour limite $\ell \in \mathbb{K}^n$ en $a$ si :
%$$ \forall \varepsilon>0, \; \exists \eta>0, \, \forall x \in I, \, \vert x-a \vert \leq \eta \Rightarrow \Vert f(x)- \ell \Vert \leq \varepsilon$$
%\end{defin}
%
%\begin{rem} On définit aussi la notion de limite en $- \infty$ et $+ \infty$ : il suffit de comprendre que la norme euclidienne \og remplace \fg la valeur absolue dans $\mathbb{R}^n$.
%\end{rem}

\section{Continuité et dérivabilité}
\subsection{Fonctions coordonnées}
\noindent Soit $\mathcal{B}=(e_1, \ldots, e_n)$ la base canonique de $\mathbb{R}^n$. Pour tout $x \in I$, il existe des réels $f_1(x), \ldots, f_n(x)$ tel que :
 $$ f(x) = (f_1(x), \ldots, f_n(x)) = \sum_{k=1}^n f_k(x) e_k$$
Les fonctions $f_k : I \rightarrow \mathbb{R}$ sont appelées les \textit{fonctions coordonnées} de $f$ dans la base canonique.
\subsection{Continuité et opérations sur les fonctions continues}
%\subsection{Continuité en un point, sur un intervalle}

\begin{defin} Soit $a \in I$. On dit que $f$ est \textit{continue} en $a$ si les fonctions coordonnées de $f$ sont continues en $a$.
\end{defin}
...
%\medskip
%
%\begin{prop}[continuité composante par composante] Soit $a \in I$. La fonction $f$ est continue en $a$ si et seulement si pour tout $k \in \Interv{1}{n}$, $f_k$ est continue en $a$. 
%\end{prop}
%
%\begin{preuve} Voir chapitre sur les espaces vectoriels normés.
%\end{preuve}
%
%\medskip

\begin{ex} La fonction $f : x \mapsto (\cos(x), \sin(x))$ est continue en tout point de $\mathbb{R}$ car les fonctions cosinus et sinus le sont.
\end{ex}

%
%\subsection{Opérations sur les fonctions continues}

\begin{prop} Soient $a \in I$, $f : I \rightarrow \mathbb{R}^n$, $g : I \rightarrow \mathbb{R}^n$ deux fonctions continues en $a$ et $\lambda \in \mathbb{R}$. Alors $(\lambda f+g)$ est continue en $a$.
\end{prop}

\begin{preuve} Il suffit de se ramener aux fonctions coordonnées.
\end{preuve}
%
\subsection{Dérivabilité et opérations sur les fonctions dérivables}
%\subsection{Dérivabilité en un point, sur un intervalle}

\begin{defin} Soit $a \in I$. On dit que $f$ est \textit{dérivable} en $a$ si ses fonctions coordonnées le sont. On définit dans ce cas le \textit{nombre dérivé} de $f$ en $a$, et on note $f'(a)$, le vecteur de $\mathbb{R}^n$ défini par :
$$ f'(a) = (f_1^{'}(a), f_2^{'}(a) ,\ldots, f_n^{'}(a))$$
\end{defin}

%la fonction 
%$$ x \mapsto \frac{f(x)-f(a)}{x-a}$$
%définie sur $I \setminus \lbrace a \rbrace$, possède une limite en $a$. Dans ce cas, cette limite (qui est un \textit{vecteur de} $\mathbb{R}^n$), est appelé \textit{vecteur dérivée} de $f$ en $a$, et est noté :
%$$ f'(a) \quad \hbox{ ou } \quad \dfrac{\textrm{d}f}{\textrm{d}x}(a)$$
%\end{defin}
%
%\begin{rem} Ainsi, $f$ est dérivable en $a$ et a pour vecteur dérivé $b \in \mathbb{R}^n$ si et seulement si :
...
\begin{rem} On définit, comme dans le cas réel, la notion de dérivabilité à gauche et à droite. Le lien entre dérivabilité et dérivabilité à gauche/droite se généralise alors facilement.
\end{rem}

%\begin{defin} Soient $f : I \rightarrow \mathbb{R}^n$ une fonction et $a$ un point intérieur de $I$ ou $a= \sup(I)$. On dit que que $f$ est \textit{dérivable à gauche} en $a$ si la restriction de
%$$ x \mapsto \frac{f(x)-f(a)}{x-a}$$
%à $I \cap ]- \infty,a]$ possède une limite en $a$.
%
%\noindent Dans ce cas, cette limite est un vecteur de $\mathbb{R}^n$ et on la note $f_g'(a)$.
%\end{defin}
%
%\begin{defin} Soient $f : I \rightarrow \mathbb{R}^n$ une fonction et $a$ un point intérieur de $I$ ou $a= \inf(I)$. On dit que que $f$ est \textit{dérivable à droite} en $a$ si la restriction de
%$$ x \mapsto \frac{f(x)-f(a)}{x-a}$$
%à $I \cap [a, + \infty[$ possède une limite en $a$.
%
%\noindent Dans ce cas, cette limite est un vecteur de $\mathbb{R}^n$ et on la note $f_d'(a)$.
%\end{defin}
%
%\medskip
%
%\noindent Notons $(f_1, \ldots, f_n)$ les fonctions coordonnées de $f$ dans la base canonique :
...
%\begin{prop}[dérivation composante par composante] Soit $a \in I$. La fonction $f$ est dérivable en $a$ si et seulement si pour tout $k \in \Interv{1}{n}$, $f_k$ est dérivable en $a$. Dans ce cas, on a :
%$$ f'(a) = (f_1^{'}(a), f_2^{'}(a) ,\ldots, f_n^{'}(a))$$
%\end{prop}
...
\begin{prop}[développement limité et dérivabilité] Soient $a \in I$ et $b \in \mathbb{R}^n$. Les assertions suivantes sont équivalentes.
...
%\end{prop}
%
%\begin{preuve} Voir chapitre sur les espaces normés.
%\end{preuve}
...
%\begin{rem} Notons $A$ la matrice de $L$ dans les bases canoniques de $\mathbb{R}^n$ et $\mathbb{R}^p$. Si on note pour tout $t \in I$, $X(t)$ le vecteur colonne formé des coordonnées de $f(t)$ dans la base canonique. La proposition précédente s'écrit :
...
\begin{defin} On dit que $f$ est \textit{dérivable} sur $I$ si $f$ est dérivable en tout point de $I$. La fonction $x \mapsto f'(x)$ est alors appelée la \textit{fonction dérivée} de $f$ et on la note $f'$.
\end{defin}

\noindent Les résultats liées aux opérations sur les fonctions dérivables (somme, composition) se généralisent sur un intervalle.
...
%\end{enumerate}
%\item Pour tout entier $k \geq 1$, on dit que $f$ est de \textit{classe} $\mathcal{C}^k$ sur $I$ si $f^{(k)}$ existe et est continue sur $I$.
%\item On dit que $f$ est de classe $\mathcal{C}^{\infty}$ sur $I$ si $f$ est de classe $\mathcal{C}^k$ sur $I$ pour tout entier $k \geq 1$.
%\end{itemize}
%\end{defin}
\begin{defin} Soient $k$ un entier naturel et $a \in I$. On dit que $f$ est de \textit{classe} $\mathcal{C}^k$ en $a$ si toutes ses fonctions coordonnées le sont. On pose alors :
...
\end{defin}
%
%\begin{prop}[classe $\mathcal{C}^k$ composante par composante] 
%\begin{itemize}
%\item La fonction $f$ est de classe $\mathcal{C}^k$ sur $I$ si et seulement si pour tout $j \in \Interv{1}{n}$, $f_j$ est de classe $\mathcal{C}^k$ sur $I$. Dans ce cas, pour tout $i \in \Interv{1}{k}$ et tout $a \in I$,
%$$ f^{(i)}(a) = (f_1^{(i)}(a), f_2^{(i)}(a) ,\ldots, f_n^{(i)}(a))$$
%\item La fonction $f$ est de classe $\mathcal{C}^{\infty}$ sur $I$ si et seulement si pour tout $j \in \Interv{1}{n}$, $f_j$ est de classe $\mathcal{C}^{\infty}$ sur $I$. Dans ce cas, pour tout $i \in \mathbb{N}^*$  et tout $a \in I$,
%$$ f^{(i)}(a) = (f_1^{(i)}(a), f_2^{(i)}(a) ,\ldots, f_n^{(i)}(a))$$
%\end{itemize}
%\end{prop}
...
%\begin{exa} Démontrer le résultat précédent par récurrence sur $k \geq 1$.
%\end{exa}
...
\noindent Pour $n=2$, on retrouve la norme usuelle :
...
\begin{defin} Soit $k \in \mathbb{N}^*$ ou $k = + \infty$.

\begin{itemize}
\item On appelle \textit{arc paramétré} de classe $\mathcal{C}^k$ tout couple $\Gamma=(I,f)$ où $I$ est un intervalle de $\mathbb{R}$ et $f : I \rightarrow \mathbb{R}^n$ une fonction de classe $\mathcal{C}^k$ sur $I$.
\item On appelle \textit{support} de l'arc paramétré l'image de $f$ par $I$, c'est-à-dire l'ensemble généralement noté $\Gamma$ défini par :
...
\begin{defip}[tangente en un point régulier]
...
%\begin{rem} Dans l'exemple précédente, on a pour tout $t \in [0,2\pi]$,
%$$\Vert f(t) \Vert^2=1$$
...
\begin{center}
$f(a)$ est un \textit{point ordinaire}
...
\begin{center}
$f(a)$ est un \textit{point de rebroussement de première espèce}
...
\begin{center}
$f(a)$ est un \textit{point d'inflexion}
...
\begin{center}
$f(a)$ est un \textit{point de rebroussement de deuxième espèce} 
...
%\begin{ex} Étudions les branches infinies en $\pm \infty$ de la courbe paramétrée définie par $f=(x,y)$ ou pour tout $t \in \mathbb{R}$,
%$$ x(t) = -4t^2+4t \; \hbox{ et } \; y(t)=-t^3+t$$
%
%\vspace{5cm}
%\end{ex}
