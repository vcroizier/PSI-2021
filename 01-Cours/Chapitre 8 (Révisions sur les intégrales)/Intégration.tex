...
\maketitle{Chapitre 8}{Rappels sur l'intégration}

\noindent Dans la suite $I$ sera un intervalle non vide et non réduit à un point, $n$ un entier naturel, $a$ et $b$ deux réels tels que $a<b$.
...
\begin{defin} Soit $f : I \rightarrow \mathbb{R}$. On dit qu'une fonction $F$ définie sur $I$ est une primitive de $f$ sur $I$ si $F$ est dérivable sur $I$ et si $F'=f$.
\end{defin}

\begin{prop} Soit $f : I \rightarrow \mathbb{R}$.
...
\noindent Nous ne redéfinissons pas la notion d'intégrale d'une fonction continue sur $[a,b]$ : nous redonnons juste les propriétés importantes.


\begin{thm} Soient $f:  I \rightarrow \mathbb{R}$ une fonction continue et $a \in I$. On pose pour tout $x \in I$,
$$ F(x) = \int_{a}^x f(t) \dt $$
Alors $F$ est l'unique primitive de $f$ s'annulant en $a$.
\end{thm}

\begin{cor} Toute fonction continue sur $I$ admet des primitives sur $I$.
\end{cor}

\begin{cor} Soient $f:  I \rightarrow \mathbb{R}$ une fonction continue. Pour tout $(a,b) \in I^2$ et toute primitive $G$ de $f$ sur $I$, on a :
$$ \int_{a}^b f(t) \dt = G(b)-G(a)$$
\end{cor}

%\begin{prop} Soient $f$ une fonction continue sur $I$ et $\alpha$, $\beta$ deux fonctions dérivables sur un intervalle $J$ et à valeurs dans $I$. Posons pour tout $x \in I$,
...
\noindent Soit $f : [a,b] \rightarrow \mathbb{R}$ une fonction continue. Pour tout $n \geq 1$, on pose :
$$S_n = \frac{b-a}{n}\som k0{n-1} f \bigg{(}  a + k \times \frac{b-a}{n} \bigg{)}$$
...
Soient $f$ une fonction continue sur $[a,b]$ et $(S_n)_{n \geq 1}$ la suite de ses sommes de Riemann. Alors : 
 $$ \lim_{n\to \pinf} S_n =  \int_a^b f(t) \dt$$
\end{thm}
...
$$ \lim_{n\to \pinf} \frac 1n \som k0{n-1} f\l(\frac kn\r) = \int_0^1 f(t) \dt$$ 
\item On peut aussi considérer les rectangles \og à droite \fg : le résultat subsiste (l'indice de la somme varie de $1$ à $n$).
\end{rems}

%\begin{ex} Déterminer un équivalent de $\dis \Sum{k=1}{n} \sqrt{k}$ quand $n$ tend vers $+ \infty$.
%
...
%Une fonction $f$ est continue par morceaux sur le segment $[a,b]$ s'il existe une subdivision \linebreak $a_0 = a < a_1 < a_2 < \dots < a_n =b$ telle que les restrictions de $f$ à chaque intervalle ouvert $]a_i,a_{i+1}[$ (avec $i \in \Interv0{n-1}$) admettent un prolongement continu à l'intervalle fermé $[a_i,a_{i+1}]$.
%\end{defin}
%
...
%\end{exems}
%
%\begin{defin} Une fonction $f$ est continue par morceaux sur un intervalle $I$ si elle est continue par morceaux sur tout segment $[a,b]$ inclus dans $I$.
%\end{defin}
%
...
%\end{rem}
%
%\begin{prop} L'ensemble des fonctions continues par morceaux sur $I$ à valeur dans $\mathbb{R}$, notée $\mathcal{CM}(I,\mathbb{R})$ est un sous-espace vectoriel de l'ensemble des fonctions de $I$ à valeurs dans $\mathbb{R}$.
%\end{prop}
%
...
\begin{prop}[Linéarité] Soient $f,g$ deux fonctions continues sur $[a,b]$ et à valeurs dans $\mathbb{R}$ et $\lambda \in \mathbb{R}$. Alors :
$$ \int_{a}^b (\lambda f(t) + g(t)) \dt = \lambda \int_{a}^b  f(t) \dt +  \int_{a}^b  g(t) \dt$$
\end{prop}

\begin{prop}[Relation de Chasles] Soient $f$ une fonction continue sur $[a,b]$ et à valeurs dans $\mathbb{R}$ et $c \in [a,b]$. Alors :
$$ \int_{a}^b f(t) \dt = \int_{a}^c f(t) \dt + \int_{c}^b f(t) \dt $$
\end{prop}

\begin{prop}[Positivité] Soient $f$ une fonction continue sur $[a,b]$ et à valeurs réelles \textit{positives}. Alors $\Int{a}{b} f(t) \dt \geq 0$.
\end{prop}
...
\begin{prop}[Croissance] Soient $f,g$ deux fonctions continues sur $[a,b]$ à valeurs réelles. Si pour tout $t \in [a,b]$, $f(t) \leq g(t)$ alors :
...
\begin{prop} Soit $f$ une fonction continue sur $[a,b]$ à valeurs dans $\mathbb{R}$. Alors $\vert f \vert$ est continue sur $[a,b]$ et à valeurs positives et :
$$ \left\vert \int_{a}^b f(t) \dt \right\vert \leq \int_{a}^b \vert f(t) \vert \dt $$
\end{prop}

%\begin{prop} Soient $f$, $g$ deux fonctions continues par morceaux sur $[a,b]$ et à valeurs dans $\mathbb{R}$. Si $f$ et $g$ coïncident sauf en un nombre fini de points alors :
%$$ \int_{a}^b f(t) \dt = \int_{a}^b g(t) \dt $$
%\end{prop}

%\section{Méthodes de calculs d'intégrales}

%\noindent Dans la suite, $I$ est un intervalle de $\mathbb{R}$.

\subsection{Intégration par parties}
\begin{thm} Soient $f,g$ deux fonctions de classe $\mathcal{C}^1$ sur $I$ à valeurs dans $\mathbb{R}$. Pour tout $(a,b) \in I^2$, on a :
...
\begin{thm} Soient $f : I \rightarrow \mathbb{R}$ une fonction continue et $\varphi$ une fonction de classe $\mathcal{C}^1$ sur un segment $[a,b]$ à valeurs dans $I$. Alors :
...
%\end{preuve}
%
%\noindent On admet le résultat suivant :
%
%\begin{thm} Soit $f : I \rightarrow \mathbb{R}$ une fonction continue par morceaux et $\varphi$ une fonction de classe $\mathcal{C}^1$ et \textit{strictement monotone} sur un segment $[a,b]$ à valeurs dans $I$. Alors :
%$$ \int_{a}^b f(\varphi(t)) \varphi'(t) \dt = \int_{\varphi(a)}^{\varphi(b)} f(x) \dx$$
%\end{thm}

\subsection{Primitives usuelles}

\noindent Le tableau suivant donne les primitives $F$ des fonctions usuelles $f$, $I$ étant le (ou les) intervalles où les fonctions et leurs primitives sont définies. Une seule primitive est donnée à chaque fois.
...
\hhline{|---|}
$\haut f(x) = \dis \frac{1}{\sqrt{x}}$ & $F(x) =2\dis \sqrt{x}$ & $\R^\ast_+ $ \\
\hhline{|---|}
...
\noindent En particulier, par dérivation d'une composée, on obtient des primitives \og à vue \fg de certaines fonctions. 
...
%
%\noindent Pour calculer une intégrale de de la forme $\dis \Int{}{} P(x) e^{ax+b} \dx$, on procède intégration par parties pour faire \og chuter le degré de $P$ \fg $\cdot$
%
%\begin{ex} Calculons $\dis \Int{0}{1} (x^2-1) e^{2x} \dx$. Les fonctions $x \mapsto \dfrac{e^{2x}}{2}$ et $x \mapsto x^2-1$ sont de classes $\mathcal{C}^1$ sur $[0,1]$ donc par intégration par parties :
...
%
%\noindent Pour calculer une intégrale de de la forme $\dis \Int{}{} e^{\beta x} \cos(ax+b)\dx$ ou $\dis \Int{}{} e^{\beta x} \sin(ax+b)\dx$, on peut effectuer deux intégrations par parties (on obtient une équation du premier degré dont l'intégrale cherchée est solution).
%
%\subsubsection{Fractions rationnelles}
%
%\noindent On appelle fraction rationnelle tout élément de la forme $\dfrac{A(X)}{B(X)}$ où $(A,B) \in \mathbb{R}[X]^2$ avec $B$ non nul. Pour déterminer calculer l'intégrale d'une fonction associée à une fraction rationnelle, on procède de la manière suivante : 
%
%\medskip
%
%\noindent $\rhd$ On effectue la division euclidienne de $A$ par $B$ : il existe un unique couple $(Q,R)$ de $\mathbb{R}[X]$ tel que :
%$$ A(X) = Q(X)B(X) + R(X)$$
...
%
%\begin{thm} Si la décomposition de $Q$ en produit de facteurs irréductibles de $\mathbb{R}[X]$ est :
...
%Celle-ci se calculant par intégration par parties successives en partant de $\dis \Int{}{} \dfrac{1}{1+x^2} \dx$
%
%\begin{ex} Déterminons $\Int{a}{b} \frac{1}{(x^2+1)^2} \dx$.
%
%\noindent Les fonctions $x \mapsto x$ et $x \mapsto  \dfrac{1}{1+x^2}$ sont de classe $\mathcal{C}^1$ sur $[a,b]$. Par intégration par parties, on a :
%\begin{align*}
...
\begin{thm}[Formule de Taylor avec reste intégrale] Soit $f : I \rightarrow \mathbb{R}$ une fonction de classe $\mathcal{C}^{n+1}$ sur $I$. Pour tout $(x,a) \in I^2$, on a :
...
%\medskip
%
%\noindent $\rhd$ Pour $n=0$, on a d'après le Théorème fondamental de l'analyse ($f$ étant de classe $\mathcal{C}^1$ sur $I$, $f'$ est continue sur $I$) :
%$$ f(x)=f(a) + \int_{a}^x f'(t) \dt$$
%ce qui est exactement l'égalité souhaitée.
%
%\medskip
%
%\noindent $\rhd$ Supposons que la propriété soit vraie pour un rang $n \in \mathbb{N}$ et soit $f$ une fonction de classe $\mathcal{C}^{n+2}$ sur $I$. Par hypothèse de récurrence, on a pour $(a,x) \in I^2$,
%$$ f(x) = \sum_{k=0}^n \frac{f^{(k)}(a)}{k!}(x-a)^k + \int_{a}^x \frac{(x-t)^n}{n!} f^{(n+1)}(t) \dt$$
...
%\begin{cor} Soit $f : I \rightarrow \mathbb{R}$ une fonction de classe $\mathcal{C}^{n+1}$ sur $I$, $(a,x) \in I^2$ et $M$ un majorant de $\vert f^{(n+1)}$ sur $[a,x]$ (ou $[x,a]$). Alors :
...
%\medskip
%
\begin{thm}[Formule de Taylor-Young] Soit $f : I \rightarrow \mathbb{R}$ une fonction de classe $\mathcal{C}^{n}$ sur $I$. Pour tout $a \in I$, on a :
...
\noindent Si $f$ est une fonction continue sur $[a,b]$, elle admet une primitive $F$ sur $[a,b]$ et on a :
...
\begin{exa} Calculer les intégrales suivantes :

\begin{multicols}{2}
\begin{enumerate}
\item $\dis \int_{-2}^{-1} \dfrac{e^{2x}}{\sqrt{1-e^{2x}}} \dx$.
\item $\dis \int_{e}^{e^2} \dfrac{1}{x\ln(x)} \dx$.
\item $\dis \int_{\pi/6}^{\pi/4} \dfrac{\tan^2(x)+1}{\sqrt{\tan(x)}} \dx$.
\columnbreak
\item $\dis \int_{0}^{\pi/4} \tan(x) \dx$.
\item $\dis \dis \int_{0}^1 \dfrac{\cos(x)}{1+\sin(x)^2} \dx$
\item $ \dis \int_{1}^{2} \dfrac{e^{-x}}{\sqrt{1-e^{-2x}}} \dx$.
\end{enumerate}
\end{multicols}
\medskip
\end{exa}
...
\noindent Si l'on souhaite calculer l'intégrale d'une fonction de la forme $x \mapsto \cos(x)^n \sin(x)^m$, on utilise les formules :
$$ \cos(x) = \frac{e^{ix}+e^{-ix}}{2} \; \et \sin(x) =\frac{e^{ix}-e^{-ix}}{2i}$$
pour linéariser l'expression de la fonction.

\medskip

\begin{ex} Calculons l'intégrale suivante :
...
\begin{exa} Calculer $\dis \int_{0}^{\pi/2} \sin(t)^4 \dt$.
\end{exa}

\subsection{Méthode 3 : par intégration par parties}

\noindent Voici quelques méthodes usuelles ($\alpha$, $\beta$, $m$ et $p$ sont des réels non nuls) :

\medskip

\begin{itemize}
\item Pour calculer l'intégrale d'une fonction de la forme $x \mapsto P(x)e^{\alpha x + \beta}$ où $P$ est une fonction polynômiale on utilise une (ou plusieurs) intégration(s) par parties : on fait tomber le degré de $P$ et on intègre la fonction exponentielle.
\item Pour calculer l'intégrale d'une fonction de la forme $x \mapsto P(x) \ln(\alpha x)$ où $P$ est une fonction polynômiale, on utilise une intégration par parties : on dérive la fonction logarithme et on intègre la fonction polynômiale.
...
\begin{exa} Calculer $I= \int_{1}^e \ln(t) \dt$ et $J = \int_{0}^1 \cos(2t) e^t \dt$.
\end{exa}

\subsection{Méthode 4 : par changement de variable}
\noindent Soit $f : I \rightarrow \mathbb{R}$ une fonction continue et $\varphi$ une fonction de classe $\mathcal{C}^1$ sur un segment $[a,b]$ à valeurs dans $I$. Alors :
$$ \int_{a}^b f(\varphi(t)) \varphi'(t) \dt = \int_{\varphi(a)}^{\varphi(b)} f(x) \dx$$
...
\begin{ex} Calculons $I=\dis \int_{1}^{e} \dfrac{\dt}{t+t\ln(t)} \cdot$
...
\begin{exa} Calculer $\dis \int_{1}^3 \dfrac{\sqrt{t} \dt}{t^2+t}$ à l'aide du changement du variable $x=\sqrt{t}$.
\end{exa}

\begin{exa}  Calculer les intégrales suivantes à l'aide d'un changement de variable :

\begin{enumerate}
\item $I= \dis \int_{0}^1 \dfrac{e^{2t}}{e^t+1} \dt$.
\item $J = \dis \int_{0}^1 \dfrac{1}{e^x+1} \dx$.
\end{enumerate}
\end{exa}

\subsection{Méthode 5 : en utilisant une fonction complexe}
\noindent Si $f$ est une fonction continue sur $[a,b]$ à valeurs dans $\mathbb{C}$, on pose :
...
\begin{ex} Calculons $I = \int_{0}^{1} \cos(t) e^t \dt$. On a :
\begin{align*}
...
\begin{exa} Calculer $\int_{0}^1 \sin(2t)e^t \dt$.
\end{exa}
...
\begin{ex} Déterminons $\dis \lim_{n \rightarrow + \infty} \sum_{k=1}^n \frac{1}{k+n}\cdot$
...
\begin{ex} Déterminons un équivalent de $\dis \Sum{k=1}{n} \sqrt{k}$ quand $n$ tend vers $+ \infty$.
...
\begin{exa} Déterminer un équivalent de $\dis \Sum{k=1}{n} k^4$ quand $n$ tend vers $+ \infty$.
\end{exa}

\begin{exa} Déterminer $\dis \lim_{n \rightarrow + \infty} \sum_{k = 1}^{n} \frac{n}{n^{2} + k^{2}} \cdot$
\end{exa}

\subsection{Savoir étudier une fonction définie par une intégrale}

\noindent La méthode suivante (présentée de manière théorique) doit être parfaitement comprise :

\medskip

\noindent Soient $f$ une fonction continue sur $I$ et $\alpha$, $\beta$ deux fonctions dérivables sur un intervalle $J$ et à valeurs dans $I$. Posons pour tout $x \in J$,
$$H(x) = \int_{\alpha(x)}^{\beta(x)} f(t) \dt$$
%Alors $H$ est bien définie et dérivable sur sur $I$ et on a pour tout $x \in  J$,
%$$ H'(x) =  \beta'(x) f(\beta(x)) - \alpha'(x) f(\alpha(x))$$
%%\end{prop} 
...
\begin{ex} Soit $H$ la fonction définie pour tout $x \in \mathbb{R}$ par :
...
\begin{exa} Justifier que la fonction $H$ définie par $H(x) = \int_{-x}^{x^2} \ln(1+t^4) \dt$ est de classe $\mathcal{C}^1$ sur $\mathbb{R}$ et donner l'expression de sa dérivée.
\end{exa}

\noindent Parfois, on demande de déterminer les limites de fonctions définies par des intégrales : une méthode type est d'encadrer \textit{subtilement} l'intégrande puis d'intégrer et d'utiliser le théorème d'encadrement.
...
\noindent Soient $a$, $b$ deux réels tels que $a \leq b$ et $(I_n)_{n \geq 0}$ est une suite définie par :
...
\begin{exa} Déterminer $\dis \lim_{n \rightarrow + \infty} \int_{0}^1 x^n e^{x} \dx$.
\end{exa}

%\begin{exa}  Pour tout entier naturel $n$, on pose $\dis I_{n}=\int_{1}^{e}x^{2}\ln \left( x\right) ^{n}dx$.
%
%\begin{enumerate}
%\item
%\begin{enumerate}
...
%\end{exa}
%
%\begin{exa} Étudier la convergence de la suite $(I_n)_{n  \geq 0}$ où pour tout $n \in \mathbb{N}$,
