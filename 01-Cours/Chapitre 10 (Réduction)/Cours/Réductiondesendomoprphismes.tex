...
\maketitle{Chapitre 10}{Réduction des endomorphismes}
...
%\medskip
%
%\noindent $\rhd$ Supposons que $D$ est stable par $u$. Par définition, on sait que pour tout $z \in D$, $u(z) \in D$ et ainsi $u(x) \in D$. Or, $x$ est non nul et appartient à $D$ donc $D = \textrm{Vect}(x)$. On obtient alors que $u(x) \in \textrm{Vect}(x)$ et donc par définition, il existe $\lambda \in \mathbb{K}$ tel que $u(x) = \lambda x$.
%
%\medskip
%
%\noindent $\rhd$ Supposons l'existence d'un scalaire $\lambda$ tel que $u(x)= \lambda x$. Soit $z \in D$. On sait que $D = \textrm{Vect}(x)$ donc il existe un scalaire $\mu$ tel que $z= \mu x$. Ainsi, par linéarité de $u$ :
...
\begin{itemize}
\item Un scalaire $\lambda \in \mathbb{K}$ est appelé \textit{valeur propre} de $u$ si \newline \phantom{il existe un vecteur $x$ de $E$, \textit{non nul}, tel que $u(x) = \lambda x$.}
\item Un vecteur $x$ de $E$ est appelé \textit{vecteur propre} de $u$ si \newline \phantom{il est \textit{non nul} et si il existe un scalaire $\lambda \in \mathbb{K}$ tel que $u(x) = \lambda x$.}
\end{itemize}
\end{defin}
...
%
%\begin{ex} Soit $x$ un vecteur propre d'un endomorphisme $u$ de $E$ associé à la valeur propre $\lambda \in \mathbb{K}$. Montrons que pour tout $\mu \in \mathbb{K}^*$, $\mu x$ est aussi un vecteur propre de $u$ associé à la valeur propre $\lambda$.
%
%\vspace{4cm}
%%
%%\noindent Tout d'abord $\mu x$ est non nul car $\mu \in \mathbb{K}^*$ et $x$ est un vecteur propre de $u$ donc non nul. On a ensuite par linéarité de $\mu$ et sachant que $x$ est un vecteur propre de $x$ associé à la valeur propre $\lambda$ :
%%$$ u(\mu x) = \mu u(x) = \mu \lambda x = \lambda (\mu x) $$
%%Donc $\mu x$ est bien un vecteur propre de $u$ associé à la valeur propre $\lambda$.
%\end{ex}
%
%\begin{rem} L'exercice précédent montre que tout multiple \textit{non nul} d'un vecteur propre de $u$ reste un vecteur propre de $u$ associé à la valeur propre $\lambda$. En particulier, il y a une infinité de vecteurs propres associés à cette valeur propre.
%\end{rem}
...
\vspace{6cm}
%\noindent Raisonnons par analyse-synthèse :
%
%\noindent $\rhd$ \textit{Analyse :} Supposons l'existence d'un scalaire $\lambda$, valeur propre de $u$. Par définition, il existe un vecteur $P$ non nul tel que $u(P(X))= \lambda P(X)$ et donc par définition de $u$, $XP(X)= \lambda P(X)$ ou encore :
%$$ (X- \lambda) P(X) = 0$$
%Or $(X- \lambda)$ est un polynôme non nul donc $P(X) = 0$ ce qui est absurde.
%
%\medskip
%
%\noindent $\rhd$ \textit{Synthèse :} le raisonnement précédent montre que l'endomorphisme $u$ n'admet pas de valeur propre.
\end{ex}
...
\medskip
%
%\begin{preuve} Le premier point est clair car $E_{\lambda}(u)$ est le noyau d'une application linéaire. Le deuxième point est clair par définition d'un vecteur propre.
%\end{preuve}
...
%\medskip
%
%\noindent Rappelons un résultat démontré dans les rappels d'algèbre linéaire : si $u$ et $v$ sont deux endomorphismes de $E$ qui commutent alors les noyaux et images de $u$ sont stables par $v$. 
%
%\medskip
...
\medskip
%
%\noindent Soient $n \geq 1$, $\lambda_1$, $\ldots$, $\lambda_n$ des valeurs propres deux à deux distinctes de $u$ et $x_1$, $\ldots$, $x_n$ des vecteurs propres respectivement associés. Soit $(\alpha_1, \ldots, \alpha_n) \in \mathbb{K}^n$ tel que :
%\begin{equation}\label{Libre}
% \alpha_1 x_1 + \cdots \alpha x_n = 0_E
% \end{equation}
%Pour tout $i \in \Interv{1}{n}$, $x_i \in E_{\lambda_i}(u)$ donc $\alpha_i x_i \in E_{\lambda_i}(u)$ (car un sous-espace propre est un sous espace-vectoriel de $E$). D'après (\ref{Libre}) et sachant qu'une somme d'une famille finie de sous-espaces propres associés à des valeurs propres deux à deux distinctes de $u$ est directe, on a que pour tout $i \in \Interv{1}{n}$, $\alpha_i x_i = 0_E$ et donc $\alpha_i = 0$ car $x_i$ est un vecteur propre donc non nul. On a donc bien montré que $(x_1, \ldots, x_n)$ était une famille libre de $E$.
%
%\vspace{5cm}
%\end{ex}
...
\medskip
%
%\noindent Soit $n \in \mathbb{N}^*$ et $a_1, \ldots, a_n$ des réels distincts.  Montrons que $f_{a_1}, \ldots , f_{a_n}$ est une famille libre de $E$. Posons $\mathcal{D}$ l'endomorphisme de $E$ définie par $\mathcal{D}(f)=f''$. On a pour tout $i \in \Interv{1}{n}$, $\mathcal{D}(f_{a_i}) = - a_i^2 f_{a_i}$ et ainsi $f_{a_i}$ (qui n'est pas la fonction nulle) est un vecteur propre de $\mathcal{D}$ associée à la valeur propre $-a_i^2$.  Les réels $a_1, \ldots, a_n$ étant distincts, il en est de même des réels $-a_1^2, \ldots, -a_n^2$ donc la famille $(f_{a_1}, \ldots, f_{a_n})$ est une famille de vecteurs propres associés à des valeurs propres deux à deux distinctes d'un endomorphisme $u$ de $E$ et est donc une famille libre de $E$. 
...
\end{exa}
%
%\begin{metho} Pour montrer qu'un scalaire $\lambda$ est valeur propre d'une matrice, il suffit de montrer que $A- \lambda I_n$ n'est pas inversible 
...
%\medskip
%
%\noindent On transpose à l'aide de cette remarque les propriétés démontrées dans le cas des endomorphismes.
%
%\begin{prop} Soient $A \in \mathcal{M}_{n}(\mathbb{R})$ et $\lambda \in \mathbb{K}$. Les assertions suivantes sont équivalentes :
%
%\begin{enumerate}
...
%\begin{ex} Soit $n \geq 1$. Déterminons les valeurs propres de la matrice $J_n$ définie par :
%$$ J_n = \begin{pmatrix}
%1 & 1 & \ldots & 1 \\
%1 & 1 & \ldots & 1 \\
%\vdots & \vdots &  & \vdots\\
...
\boxput*(0,1){
\colorbox{white}{\textit{{\large Rappels sur les polynômes}}}
}{
...
\noindent $\rhd$ Soient $P \in \mathbb{K}[X]$ non nul, $\alpha \in \mathbb{K}$ et $k \in \mathbb{N}^*$. Les assertions suivantes sont équivalentes :

\begin{itemize}
\item $(X- \alpha)^k \vert P$ et $(X- \alpha)^{k+1} \nmid P$.
\item $P(\alpha) = P'(\alpha) = \cdots = P^{(k-1)}(\alpha) = 0$ et $P^{(k)}(\alpha) \neq 0$.
\end{itemize}
Si les conditions sont vérifiées, on dit que $k$ est l'\textit{ordre de multiplicité} de $\alpha$ en tant que racine de $P$.

\medskip

\noindent $\rhd$ Si $P$ est non constant, on dit qu'il est \textit{scindé} sur $\mathbb{K}$ si il existe des scalaires $\lambda_1$, $\ldots$, $\lambda_n$ ($n \geq 1$) et un scalaire $\mu$ tel que :
...
\noindent $\rhd$ Rappelons que tout polynôme est scindé dans $\mathbb{C}$ mais que ce n'est pas le cas dans $\mathbb{R}$ (par exemple $P(X)=X^2+1$).

 \end{minipage}}}
 
 \medskip
 
 \begin{defin} Soit $u \in \mathcal{L}(E)$ (ou $A \in \mathcal{M}_n(\mathbb{K})$). On appelle \textit{multiplicité} de $\lambda$ en tant que valeur propre de $u$ (resp. $A$), l'ordre de multiplicité de $\lambda$ en tant que racine de $\chi_u$ (resp. $\chi_A$). On notera dans ce cours $m(\lambda)$ pour la multiplicité de $\lambda$.
 \end{defin}
...
%Donc $\chi_u(0) =(-1)^n \dis \prod_{k=1}^n \lambda_k$. Or $\chi_u(0)$ est le coefficient constant de $\chi_u$ et on a montré que celui-ci est égal à $(-1)^n \textrm{det}(u)$. On obtient donc que :
...
%\medskip
%
%\noindent Supposons que $\textrm{Mat}_{\mathcal{B}}(u)$ soit diagonale : il existe alors $\lambda_1$, $\lambda_2$, $\ldots$, $\lambda_n$ tels que :
...
%0 & 0 & \ldots & \lambda_n
%\end{pmatrix}$$
%Nécessairement tous les éléments de cette famille sont différents du vecteur nul (sinon la famille ne serait pas libre) et pour tout $i \in \Interv{1}{n}$, $f(e_i) = \lambda_i e_i$ donc la famille $\mathcal{B}$ est bien une base de vecteurs propres.
%
%\medskip
%
%\noindent Réciproquement, si $\mathcal{B}$ est une base de vecteurs propres, on obtient une matrice diagonale (même raisonnement).
%\end{preuve}
%
%\begin{thm} Soit $u \in \mathcal{L}(E)$. Les assertions suivantes sont équivalentes :
%
...
%\medskip
%
%\noindent $\rhd$ Supposons que $u$ est diagonalisable. Par définition, il existe une base $(e_1, \ldots, e_n)$ de $E$ formée de vecteurs propres de $u$. On sait que la somme de sous-espaces propres associés à des valeurs propres distinctes est directe et on sait que tout vecteur propre appartient à un sous-espace propre donc pour tout $i \in \Interv{1}{n}$,
...
%\medskip
%
%\noindent $\rhd$ Supposons maintenant que :
...
%1 & 0  & -1 \\ 
%\end{pmatrix}$$
%La matrice $P$ est inversible car c'est la matrice de passage de la base canonique de $\mathcal{M}_{3,1}( \mathbb{R})$ vers la base formée des vecteurs propres obtenus (\textit{c'est une base par concaténation}). On a :
%$$ A = P D P^{-1}$$
...
\medskip
%
%\noindent $\rhd$ Supposons que $u$ est diagonalisable. Notons $\lambda_1$, $\ldots$, $\lambda_k$ ($k \geq 1$) les valeurs propres de $u$. On sait d'après la preuve du théorème précédent que :
...
%Donc $\chi_u$ est scindé et pour tout $i \in \Interv{1}{k}$,
%$$ m(\lambda_i) = \alpha_i = \textrm{dim}(E_{\lambda_i}(u))$$
%
%\medskip
%
%\noindent $\rhd$ Supposons que $\chi_u$ est scindé sur $\mathbb{K}$ et pour tout $\lambda \in \textrm{Sp}(u)$, $\textrm{dim}(E_{\lambda}(u))= m(\lambda)$. Le polynôme $\chi_u$ étant scindé, la somme des mes multiplicités de ses racines est égal à son degré qui vaut $n$ donc :
...
%
%\begin{defin} Soit $A \in \mathcal{M}_n(\mathbb{K})$. On dit que $A$ est \textit{diagonalisable} si $A$ est semblable à une matrice diagonale : c'est-à-dire si il existe $D \in  \mathcal{M}_n(\mathbb{K})$ et $P \in  \mathcal{GL}_n(\mathbb{K})$ tels que $A=PDP^{-1}$.
%\end{defin}
%
...
%\begin{rem} Si $A= PDP^{-1}$ avec $P$ inversible et $D$ diagonale alors :
%
%\begin{itemize}
...
\begin{prop}[hors-programme à savoir retrouver] Soient $u \in \mathcal{L}(E)$ et $P \in \mathbb{K}[X]$ un polynôme annulateur de $u$. Alors toute valeur propre de $u$ est racine de $P$. 
\end{prop}
...
\vspace{7cm}
 ...
 \begin{thm}[Cayley-Hamilton] Soit $u \in \mathcal{L}(E)$ et $A \in \mathcal{M}_n(\mathbb{K})$. Alors :
 \begin{itemize}
...
\end{itemize}
 ...
 \vspace{7cm}
% 
%\noindent On a $\chi_A(X) = (X-3)^2(X-1)$ donc $(X-3)^2(X-1)$ est un polynôme annulateur de $A$. Les valeurs propres de $A$ sont $3$ et $1$ donc tout polynôme annulateur est divisible par $(X-3)(X-1)$ (en fait, $(X-3)(X-1)$ est un polynôme annulateur de $A$ ce qui donne un polynôme annulateur avec un degré plus petit que $\chi_A$).
 \end{ex}
 ...
 \medskip
% 
% \noindent $\rhd$ Supposons que $u$ est diagonalisable. Notons $\lambda_1$, $\ldots$, $\lambda_p$ ($1 \leq p \leq n$) les valeurs propres deux ) deux distinctes de $u$. L'application $u$ étant diagonalisable, il existe une base de vecteurs propres $(e_1, \ldots, e_n)$ de $u$. Posons :
