...
\begin{document}
\Titre{PSI}{Promotion 2021--2022}{Mathématiques}{TD 9 : Intégrales généralisées}
...
%\begin{multicols}{3}
%\begin{enumerate}
%\item $I_1=\int_{0}^{+ \infty} e^{-x^2} \dx$.
%\item $I_2= \int_{-\pi/2}^{\pi/2} \ln(1+ \sin(x)) \dx$
%\item $I_3= \int_{0}^{+ \infty} \dfrac{1+\sin(t)}{1+\sqrt{t^3}} \dt$
%\item $I_4=  \int_{0}^{1} -e^{-x} \ln(x)  \dx$
%\item $I_5=  \int_{0}^{+ \infty} \dfrac{1}{ch(t) - \cos(t)} \dt$
%\item $I_6= \int_{0}^{1} \dfrac{\ln(x)}{\sqrt{x}} \dx$
%\item $I_7 = \int_{1}^{+ \infty} \frac{\sin(x)}{x^2} \dx$
%\item $I_8 = \int_{2}^3 \frac{\sin \left( \frac{1}{x-2} \right)}{\sqrt{(x-2)(3-x)}}  \dx$
%\item $I_{9} = \int_{1}^{+ \infty} \frac{1}{t} \left(e^{\frac{1}{t}}- \cos \left( \frac{1}{t} \right) \right) \dt \; $
%\item $I_{10} = \int_{1}^{+\infty}  \frac{1}{x} \sin \left( \frac{1}{x} \right) \dx$
%\item $I_{11} = \int_{1}^{ + \infty} \frac{\arctan(t)}{t^2} \dt$.
%\item $I_{12} = \int_{1}^{+ \infty} \frac{1}{(x-1)^a (x+1)} \dx$ 
%\item $I_{13} = \int_{0}^{\pi} \frac{\dx}{(1-\cos(x))^a}$
%\item $I_{14} = \int_{0}^{\pi/2} \ln(\sin(x)) \dx$
%\item $I_{15} = \int_{1}^{3} \frac{1}{\sqrt{x^3-1}} \dx$
%\end{enumerate}
%\end{multicols}
...
%
%
%\exo Calculer $\lim_{n \to + \infty} \int_{0}^{ + \infty} \frac{n!}{\prod_{k = 1}^{n} {(k + x)}} \dx$.


%\exo  \begin{enumerate}
%  \item
%    Démontrer la convergence de la série de terme général $a_{n} = \frac{n!}{n^{n}}\cdot$
%  \item
