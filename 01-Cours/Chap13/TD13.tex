...
\begin{document}
\Titre{PSI}{Promotion 2021--2022}{Mathématiques}{TD 13 : Fonctions définies par une intégrale}
...
\begin{Exercice}[$\bigstar$] Soit $f : x \mapsto \int_{0}^{+ \infty} \dfrac{\arctan(xt)}{t(1+t^2)} \dt$.
\begin{enumerate}
...
\begin{Exercice}[$\bigstar$] On considère la fonction $f : x \mapsto \int_0^{+ \infty} \dfrac{e^{-xt}}{\sqrt{1+t}} \dt$.
\begin{enumerate}
...
%%\exo
%%Soit $f$ la fonction définie sur $\mathbb{R}$ par ${f(x)=\int_x^{2x} e^{-t^2}\;\text{d}t}$.
%%\begin{enumerate}
...
%%\item En encadrant $f(x)$, déterminer ${\lim_{x \rightarrow +\infty} f(x)}$.
%%\end{enumerate}
%
%\exo
%Soit $f$ la fonction définie par ${f(x)=\frac 1 {x-1}\int_1^x \frac{t^2}{t^8+1} \;\text{d}t.}$
%\begin{enumerate}
