...
\begin{Exa}[\ding{80}] Montrer que si $u$ est une isométrie de $E$, espace euclidien, alors $\textrm{Ker}(u-\textrm{Id})$ et $\textrm{Im}(u-\textrm{Id})$ sont supplémentaires et orthogonaux. Calculer pour tout $x \in E$,
...
\begin{Exa}[\ding{80}] Soient $f$ un endomorphisme symétrique d'un espace euclidien $E$ et $(a,b) \in \mathbb{R}^2$ tel que $a \leq b$. Montrer que :
...
\begin{Exa}[\ding{80}] Résoudre l'équation $X ~^tX X= I_n$ d'inconnue $X \in \mathcal{M}_n(\mathbb{R})$.
\end{Exa}
...
\begin{Exa}[\ding{80}] Soit $f$ un endomorphisme symétrique d'un espace euclidien $E$. Posons :
  \[
...
\begin{Exa}[\ding{80}] Soit $\theta \in \mathbb{R} \setminus \lbrace p \pi \, \vert \, p \in \mathbb{Z} \rbrace \cdot$
\begin{enumerate}
