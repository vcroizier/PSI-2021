...
% Pour tout $\lambda \in \mathbb{K}$ et pour tout $x \in E$, $N(\lambda x) = \vert \lambda \vert N(x)$ (\textit{homogénéité}).
 \item \textit{Séparation} :
 
% Pour tout $x \in E$, $N(x)=0$ si et seulement si $x=0_E$ (\textit{séparation}).
 \item \textit{Inégalité triangulaire} :
 
% Pour tout $(x,y) \in E^2$, $N(x+y) \leq N(x)+ N(y)$ (\textit{inégalité triangulaire}).
 \end{itemize}
...
\noindent Pour tout $x=(x_1, x_2, \ldots, x_n) \in \mathbb{K}^n$, on pose :
...
%\medskip
%
%\noindent $\rhd$ Montrons uniquement l'inégalité triangulaire pour la norme infini (les autres propriétés étant simples à montrer) : soit $(x,y) \in (\mathbb{K}^n)^2$. On a pour tout $i \in \Interv{1}{n}$, d'après l'inégalité triangulaire,
...
\begin{exa} Montrer que la norme infini est une norme sur $\mathbb{K}^n$.
\end{exa}

\subsubsection{Exemple 4 : $E$ est un ensemble de fonctions bornées}

\noindent Soient $I$ un intervalle non vide de $\mathbb{R}$ et $\mathcal{B}(I,K)$ l'ensemble des fonctions bornées de $I$ dans $\mathbb{K}$ (qui est un sous-espace vectoriel de $\mathcal{F}(I, \mathbb{K})$). Pour tout $f \in \mathcal{B}(I,K)$, on pose :
$$ \Vert f \Vert_{\infty} = \sup_{x \in I} \vert f(x) \vert $$
On définit ainsi la \textit{norme infini} (ou \textit{norme de la convergence uniforme}). C'est une norme sur $\mathcal{B}(I,K)$ (voir cours sur les suites et séries de fonctions).
...
%
%\begin{rem} Si $d : E^2 \rightarrow \mathbb{R}_+$ est une application vérifiant les trois points précédents, on dit aussi que $d$ est une distance sur $E$ mais elle ne provient pas nécessairement d'une norme
...
\begin{ex} Représentons graphiquement les boules unités ouvertes de $\mathbb{R}^2$ pour les normes $1$, $2$ et $\infty$.
...
\noindent $\rhd$ Si $x \geq 0$, cela est équivalent à \phantom{$-1-x < y < 1-x$.}

\noindent $\rhd$ Si $x<0$, cela est équivalent à \phantom{$-1+x<y<1+x$.}
...
\begin{defin} Soit $E$ un espace vectoriel. 

On appelle \textit{suite} d'éléments de $E$ toute application $u : \mathbb{N} \rightarrow E$. Pour tout $n \in \mathbb{N}$, on note $u_n$ au lieu de $u(n)$ : ce vecteur est le terme de rang $n$ de cette suite. On note la suite $(u_n)_{ n \geq 0}$ ou $(u_n)$.
\end{defin}
...
\begin{enumerate}
\item Si $A$ est une partie de $E$, on dit que $A$ est \textit{bornée} si il existe un réel $M \geq 0$ tel que pour tout $a \in A$, $\Vert a \Vert \leq M$, ce qui équivalent à $A \subset B_f(0_E,M)$.
\item Si $(u_n)_{n \geq 0}$ est une suite de $E$, on dit que $(u_n)_{n \geq 0}$ est \textit{bornée} si $\lbrace u_n \, \vert \, n \geq 0\rbrace$ est bornée.

% ce qui équivalent à l'existence d'un réel $M \geq 0$ tel que pour tout $n \geq 0$, $\Vert u_n \Vert \leq M$.
\item Si $X$ est un ensemble non vide et $f : X \rightarrow E$ une application, on dit que $f$ est \textit{bornée} si $\lbrace f(x) \, \vert \, x \in X \rbrace$ est bornée.
...
\noindent Ainsi, $(u_n)_{n \geq 0}$ est bornée si et seulement si :

\vspace{1cm}

\noindent et $f : X \rightarrow E$ est bornée si et seulement si :

\vspace{1.3cm}

\begin{thm} Les définitions précédentes ne dépendent pas de la norme utilisée car $E$ est de dimension finie.
\end{thm}

\begin{preuve} Conséquence directe du fait que les normes sont équivalentes sur $E$.
\end{preuve}
...
%Or $\dis \lim_{x \rightarrow 0}  -\ln(x) -1  + \sin(1) = + \infty$ donc la définition de $f$ bornée ne peut être vérifiée pour aucun réel positif $M$.
\end{exems}
...
\vspace{6cm}
%\noindent Fixons $a \in E$ et $r \in \mathbb{R}_+$. Soient $(x,y) \in B(a,r)^2$ et $\lambda \in [0,1]$. On a :
%
%\begin{align*}
...
%\medskip
%
%\noindent $\rhd$ Considérons la norme 1 : il semble intuitif de dire que cette suite converge vers $(0,0)$. Pour tout $n \geq 1$,
...
%\medskip
%
%\noindent $\rhd$ Considérons la norme infini : il semble intuitif de dire que cette suite converge vers $(0,0)$. Pour tout $n \geq 1$,
...
%\noindent La définition de convergence dépend à priori de la norme employée sur l'espace. Le théorème suivant (que nous admettons) est crucial pour la suite.
%
%\begin{thm} La notion de convergence de suite ne dépend pas de la norme choisie si l'on travaille sur un espace vectoriel de \textit{dimension finie}.
%\end{thm}
...
%\begin{preuve} Nous commençons par montrer un résultat préliminaire puis nous montrons le résultat pas double implications.
%
%\medskip
%
%\noindent $\rhd$ Tout vecteur $x$ de $E$ a des coordonnées $(x_1, \ldots, x_p)$ dans cette base $\mathcal{B}$. On pose alors :
...
%\medskip
%
%\noindent $\rhd$ Supposons que la suite $(u_n)_{n \geq 0}$ converge vers un vecteur $\ell = (\ell_1, \ldots, \ell_p)$ de $E$.
%
%
...
%\medskip
%
%\noindent $\rhd$ Supposons que pour tout $i \in \Interv{1}{p}$, la suite $(u_{n,i})_{n \geq 0}$ converge vers un scalaire $\ell_i$.
%
...
\end{defin}
%
%\begin{prop} On admet que si $E$ est de dimension finie, la notion de point intérieur et la notion d'ouverts ne dépend pas de la norme choisie. On ne précisera donc pas la norme.
%\end{prop}
...
%
%\noindent Posons $\varepsilon = r - \Vert b-a \Vert$. Alors $\varepsilon>0$ car $b \in B(a,r)$ et pour tout $x \in B(b, \varepsilon)$, on a :
...
%\begin{thm} On admet que toutes les définitions et résultats de cette section ne dépendent de la norme car $E$ est de dimension finie.
%\end{thm}
...
%
%\noindent $\rhd$ Supposons que le point $a$ est adhérent à $A$. Pour tout entier naturel $n \geq 1$, on a $B(a, \frac{1}{n}) \cap A \neq \varnothing$ et ainsi il existe $x_n \in B(a,\frac{1}{n}) \cap A$. On a donc pour tout $n \geq 1$,
...
%\medskip
%
%\noindent $\rhd$ Supposons l'existence d'une suite $(x_n)_{n \geq 0}$ d'éléments de $A$ qui converge vers $a$. Fixons $r > 0$. Par définition de la convergence, pour $n$ assez grand, on a :
%$$ \Vert x_n - a \Vert < r $$
...
\end{rem}
%
%\begin{defin} Soient $m \in \mathbb{R}$, $f$ une fonction définie sur $]- \infty, m[$ à valeurs dans $F$ et $b \in F$.
%
%\noindent On dit que $f$ a pour limite $b$ en $- \infty$ si :
...
%\noindent $\rhd$ Supposons que la limite de $f$ en $a$ existe et soit égale à $\ell$ et fixons une suite $(a_n)_{n \geq 0}$ d'éléments de $A$ convergente vers $a$. 
%
%\newpage
%$\phantom{test}$
...
%%
%%\noindent On a ainsi montré que $(f(a_n))_{n \geq 0}$ convergeait vers $\ell$.
%
%
%\vspace{7cm}
%
%\noindent $\rhd$ Supposons que pour toute suite $(a_n)_{n \geq 0}$ d'éléments de $A$ convergente vers $a$, $(f(a_n))_{n \geq 0}$ converge vers $\ell$. Supposons par l'absurde que $f$ n'admette pas $\ell$ pour limite en $a$.
%%
%% On a alors :
%%$$ \exists \varepsilon > 0, \, \forall r > 0, \, \exists x \in A, \, \Vert x-a \Vert_E < r \hbox{ et } \Vert f(x) - \ell \Vert_F > \varepsilon$$
%%Ainsi, pour tout entier $n \geq 1$ en posant $r = \dis \frac{1}{n}$, il existe $x_n \in A$ tel que $\Vert x_n-a \Vert_E < r$ et vérifiant $\Vert f(x_n) - \ell \Vert_F > \varepsilon$. La suite $(x_n)_{n \geq 1}$ est donc une suite d'éléments de $A$ convergeant vers $a$ (conséquence du théorème d'encadrement) et telle que $(f(x_n))_{n \geq 1}$ ne converge pas vers $\ell$ car pour tout $n \geq 1$,
...
%\medskip
%
%\begin{ex} Soit $f : \mathbb{R}^2 \rightarrow \mathbb{R}^3$ définie par :
...
\newpage
\begin{ex} Montrons que le cercle unité $\mathbb{U} = \lbrace (x,y) \in \mathbb{R}^2 \, \vert \, x^2+y^2=1 \rbrace$ est un fermé de $\mathbb{R}^2$.

\vspace{4cm}

%\medskip
%
%\noindent L'application $f : \mathbb{R}^2 \rightarrow \mathbb{R}$ définie par :
%$$ f((x,y)) = x^2+y^2-1 $$
...
\begin{defin} Un ensemble fermé et borné d'un espace vectoriel de dimension finie est appelé un \textit{compact}.
\end{defin}

\begin{thm} Soient $K$ une partie compacte de $E$ (de \textit{dimension finie}) et $f : K \rightarrow \mathbb{R}$ une fonction continue. Alors $f$ est bornée et atteint ses bornes.
\end{thm}

\medskip

\noindent 
\begin{exa} Montrer que la fonction $f : \mathbb{R}^2 \rightarrow \mathbb{R}$ définie par $f((x,y))= xy$ est bornée et atteint ses bornes sur le cercle unité $\mathbb{U}$.
\end{exa}

\subsection{Les applications lipschitziennes}

\begin{defin} Soit $k \in \mathbb{R}_+$. On dit que $f : E \rightarrow F$ est $k$-\textit{lipschitzienne} si :
$$ \phantom{\forall (x,y) \in E^2, \quad \Vert f(x)-f(y) \Vert_F \leq k \Vert x-y \Vert_E}$$
On dit que $f$ est \textit{lipschitzienne} si il existe $k \in \mathbb{R}_+$ tel que $f$ soit $k$-lipschitzienne.
\end{defin}
...
\vspace{5cm}
%
%\noindent La fonction $\ln$ est dérivable sur $[1, + \infty[$ et pour tout $x \in [1, + \infty[$,
...
%\medskip
%
%\noindent Posons $\eta = \dfrac{\varepsilon}{k}$. Pour tout $x \in E$ vérifiant $\Vert x-a \Vert_E \leq \eta$, on a alors :
%\begin{align*}
...
%\medskip 
%
%\noindent L'application $u$ est une application linéaire entre deux espaces vectoriels de dimension finie donc $u$ est continue et l'on a :
%$$ \textrm{Ker}(u) = \lbrace x \in E \, \vert \, u(x)=0 \rbrace$$
%donc le noyau de $u$ est fermé.
%
%\medskip
%\noindent Par exemple, l'ensemble des matrices carrées $\mathcal{M}_n(\mathbb{K})$ de trace nulle est un fermé de $\mathcal{M}_n(\mathbb{K})$.
